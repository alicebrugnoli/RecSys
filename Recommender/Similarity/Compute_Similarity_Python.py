...
import Recommender.Recommender_utils as recommender_utils
        ...
        for data_point_index in range(len(row_list_to_add)):
            ...
            if self._next_cell_pointer == len(self._row_array):
                self._row_array = np.concatenate(
                    (self._row_array, np.zeros(self._additional_data_block, dtype=self._dtype_coordinates)))
                self._col_array = np.concatenate(
                    (self._col_array, np.zeros(self._additional_data_block, dtype=self._dtype_coordinates)))
                self._data_array = np.concatenate(
                    (self._data_array, np.zeros(self._additional_data_block, dtype=self._dtype_data)))
...
class Compute_Similarity_Python:
    ...
    def __init__(self, dataMatrix, topK=50, shrink=100, normalize=True,
    ...
    def compute_similarity(self, start_col=None, end_col=None, block_size=100):
        ...
        similarity_builder = Incremental_Similarity_Builder(self.n_columns,
                                                            initial_data_block=self.n_columns * self.topK,
                                                            dtype=np.float32)
        ...
        while start_col_block < end_col_local:
            ...
            for col_index_in_block in range(this_block_size):
                ...
                if self.normalize:
                    ...
                    if self.asymmetric_cosine:
                        denominator = sum_of_squared_to_alpha[
                                          columnIndex] * sum_of_squared_to_1_minus_alpha + self.shrink + 1e-6
                    else:
                ...
                # Apply the specific denominator for Tanimoto
                    ...
                    denominator = sum_of_squared[
                                      columnIndex] + sum_of_squared - this_column_weights + self.shrink + 1e-6
                    this_column_weights = np.multiply(this_column_weights, 1 / denominator)
            ...
            if time.time() - start_time_print_batch >= 300 or end_col_block == end_col_local:
                ...
                print("Similarity column {} ({:4.1f}%), {:.2f} column/sec. Elapsed time {:.2f} {}".format(
                    processed_items, processed_items / (end_col_local - start_col_local) * 100, column_per_sec,
